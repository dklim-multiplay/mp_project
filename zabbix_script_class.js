...
li9.setAttribute("style", "margin-top: 2%");
li8.parentNode.appendChild(li9);
...
function formatting_button(){
    ...
    for(var i = 0; i < split_machine.length; i++){
        split_data = split_machine[i].split(/(,| )/);
        ...
        for(var j = 0; j < split_data.length; j++){
            ...
            else if(split_data[j].match(/([aA]dministrator|root)/)){
            ...
            //else if(split_data[j].match(/[\w[!@#$%^&\*\(\)\-\=\_\+\;\'\:]?]{8,16}/)){//////////////testing
            //else if(!split_data[j].match(/((UNI\d+)|([aA]dministrator|root|admin)|(\d{1,3}.){3}(\d{1,3})|([\w]*-){2}[\d]*)/)){
    ...
    console.log("UNI = " + my_uni);
    console.log("username = " + my_username);
    console.log("myhostname = " + my_hostname);
    console.log("myip = " + my_ip);
    console.log("mypassword = " + my_password);
    console.log("machine length = " + machine_list.length);

    for(var k=0; k < machine_list.length; k++){
        console.log("print Activision " + machine_list[k].printActivisionCSV());
    }
