...
function getGameImages(){
    ...
    var url = "https://gameforge.multiplay.co.uk/events/Online/toolbar.html?sectionid=6;section=;modid=0;admin=0";
    GM.xmlHttpRequest({
        method: 'GET',
        url: url,
        headers: {
...
function getImages(doc){
    ...
    */
    ...
    for(var i = 25; i < 135; i++){
        ...
        //for(var j=0; j < table_string.length; j++){
        //	console.log("j= " + j + " " + table_string[j]);
        //}
        split_string = table_string.split("\"");
        //console.log(split_string);
        for(var j=0; j < split_string.length; j++){
            //if(split_string[j] == "<a title="){
            if(split_string[j] == " title="){
                //console.log("map_key = "+ split_string[j+1]);////////////????
                map_key = split_string[j+1];
                key_array.push(map_key);
            }
            if(split_string[j].includes("/cgi-adm/servers.pl?opt=ServersAdminList;sectionid=") == true){
                section_string = split_string[j].replace("/cgi-adm/servers.pl?opt=ServersAdminList;sectionid=","");
                //console.log("map_value = "+section_string.substring(0,4));////////////????
...
/*
        ...
        section_key = Array.from(map.keys())[i];
        console.log(section_key);
        option.value = option.text = section_key;
